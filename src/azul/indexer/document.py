...
class Contribution(Document[ContributionCoordinates[E]]):
    source: SourceRef[SimpleSourceName, SourceRef]
    ...
    def field_types(cls, field_types: FieldTypes) -> FieldTypes:
        return {
            **super().field_types(field_types),
            'source_id': pass_thru_str,
            'source_name': pass_thru_str,
    ...
    def _from_json(cls, document: JSON) -> Mapping[str, Any]:
        return dict(super()._from_json(document),
                    source=SourceRef(id=document['source_id'],
                                     name=SimpleSourceName(document['source_name'])))
...
class Aggregate(Document[AggregateCoordinates]):
    ...
    @classmethod
    def _from_json(cls, document: JSON) -> Mapping[str, Any]:
        def entity_to_source_ref(entity):
            return SourceRef(id=entity['id'], name=SimpleSourceName(entity['name']))

        return dict(super()._from_json(document),
                    num_contributions=document['num_contributions'],
                    sources=map(entity_to_source_ref, document['sources']),
