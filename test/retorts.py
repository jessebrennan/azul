...
class AuthResponseHelper:
    """
    Work around the lack of reentrance in the `responses` library.`Responses`
    suffers from an inherent design flaw: it uses unittest.mock.patch which
    is global but its decorators and context managers pretend to be reentrant.
    It is essentially impossible to nest a RequestMock CM inside another one,
    or in a method decorated with @responses.activate.
    ...
    restore the set of pass-throughs to their original value, essentially
    undoing all .add() and .add_passthru() invocations.
    """

    def __init__(self,
                 passthru_url: str = None,
                 request_mock: Optional[responses.RequestsMock] = None) -> None:
